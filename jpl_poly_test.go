...
func polyTchebT(n int, x float64) float64 {
	if 0 > n {panic("invalid parameter negative n")}
	if -1 > x || 1 < x {panic("invalid parameter x")}
	var p float64
	switch n {
		case  0: p=1
		case  1: p=x
		default: p=2*x*polyTchebT(n-1,x) - polyTchebT(n-2,x)
	}
	return p
}
func polyTchebU(n int, x float64) float64 {
	...
	switch n {
		case  0: p=1
		case  1: p=2*x
		default: p=2*x*polyTchebU(n-1,x) - polyTchebU(n-2,x)
	}
	return p
}
